...
export function CalendarExplorer({ camps }: CalendarExplorerProps) {
  const [selectedCategory, setSelectedCategory] = useState<CampCategory | "ALL">("ALL");
  ...
  const filteredCamps = useMemo(() => {
    return camps.filter((camp) => {
      if (selectedCategory !== "ALL" && camp.category !== selectedCategory)
        return false;
      if (selectedType !== "ALL" && camp.campType !== selectedType) return false;
  ...
  }, [camps, selectedCategory, selectedAge, selectedType]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map(([key, label]) => (
                <button
                  key={key}
                  onClick={() => setSelectedCategory(key as CampCategory | "ALL")}
                  className={cn(
                    "filter-chip text-xs",
                    selectedCategory === key ? "filter-chip-active" : "filter-chip-inactive"
                  )}
                >
                  {label}
                </button>
              ...
              <div className="flex flex-wrap gap-2">
                {campTypes.map(([key, label]) => (
                  <button
                    ...
                    className={cn(
                      "filter-chip text-xs",
                      selectedType === key ? "filter-chip-active" : "filter-chip-inactive"
                    )}
                  >
                    {label}
                  </button>
              ...
              <div className="flex flex-wrap gap-2">
                {AGE_FILTERS.map((age) => (
                  <button
                    ...
                    className={cn(
                      "filter-chip text-xs",
                      selectedAge.label === age.label ? "filter-chip-active" : "filter-chip-inactive"
                    )}
                  >
                    {age.label}
                  </button>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-4 text-xs text-bark-300 animate-fade-up stagger-2">
        ...
        {[
          ["Sports", "bg-terracotta-400/70"],
          ["Arts", "bg-amber-300/70"],
          ["STEM", "bg-sky-400/70"],
          ["Nature", "bg-pine-500/70"],
          ["Theater", "bg-rose-500/70"],
          ["Cooking", "bg-orange-400/70"],
          ["Multi-Activity", "bg-pine-300/70"],
        ].map(([label, color]) => (
          <span key={label} className="flex items-center gap-1.5">
            <span className={`w-4 h-3 rounded ${color}`} />
            {label}
          </span>
        ))}
      </div>
