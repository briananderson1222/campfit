...
// same pattern out to campfit's ADRs. Promotion into a reusable Builder Kit
// skill is deferred to a follow-up issue — this copy is intentionally
// campfit-scoped (see SUBJECT_GROUPS below) rather than fully generic.
//
...
// --- Subject grouping (campfit-specific, reviewed by hand) ------------------
//
// campfit has no single root CONTEXT.md. Vocabulary instead lives in a DDD
// multi-context structure the owner already authored: CONTEXT-MAP.md plus one
// docs/contexts/<name>/CONTEXT.md per bounded context. Per the multi-repo
// rollout instruction, that existing structure is read-only/not restructured
// here, so each subject below is a noun from the relevant per-context
// CONTEXT.md rather than a root one.
//
// Groupings chosen for campfit's 2 ADRs — both match existing per-context
// CONTEXT.md terms verbatim, so no vocabulary file needed a new term:
//   - 0001 (session registration status is the domain truth) -> "Registration
//     Status", matching docs/contexts/camp-discovery/CONTEXT.md's term of the
//     same name (Registration Status belongs to Session; a Camp-level view is
//     only a derived Camp Registration Summary).
//   - 0002 (verified camps and sessions are aggregates of verified claims)
//     decides one aggregate-verification principle applied to two distinct
//     subjects, so it is evidence for two stubs, matching
//     docs/contexts/trust-review-provenance/CONTEXT.md's two paired terms:
//       - "Verified Camp Claim Set" (identity, location, description,
//         classification, contact/registration-path claims).
//       - "Verified Session Claim Set" (session dates, session time,
//         eligibility, registration status, price options, registration
//         path).
const SUBJECT_GROUPS = [
  ...
  {
    slug: "verified-camp-claim-set",
    subject: "Verified Camp Claim Set",
    adrNumbers: [2],
  },
  {
    slug: "verified-session-claim-set",
...
// campfit's two ADRs have no formal "Date:"/"(YYYY-MM-DD)" field at all, so
// both fall back to "unknown" in the generated index — a correct reflection
// of the source material, not a script bug. This only affects generated
// index display, never the frozen ADR content.
