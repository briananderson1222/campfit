 ...
 *     `runTraversePipelineForSource` auto-retries ONCE with the render
 *     FetchLike, and the RENDERED attempt's extraction recovers the
 *     hydrated marker the plain fetch could never see. `shellEscalation`
 *     records the escalation + both attempts' warnings/item counts.
...
async function testShellDetectedAutoRetryRecoversMarker(baseUrl: string) {
  ...
  assert.ok(result.render, "a successful render retry must carry the same render telemetry a render: true source gets");
  ...
  console.log("✓ js-shell-suspected auto-retries with a render and the rendered attempt's recovered marker wins");
}
...
async function testEmbeddedStateAvailableSkipsRender(baseUrl: string) {
  ...
  assert.equal(result.render, undefined, "no render should ever occur for the downgraded (embedded-state-available) case");
...
async function testRenderRetryFailureKeepsFirstAttempt(baseUrl: string) {
  ...
  assert.equal(result.render, undefined, "no render telemetry when the render retry never completed");
...
async function testRenderTrueSourceNeverDoubleRetries(baseUrl: string, renderRequestCounts: Map<string, number>) {
  ...
  assert.ok(result.render, "render: true telemetry is unaffected");
  assert.equal(after - before, 1, "a render: true source must be rendered exactly ONCE, never double-rendered");

  console.log("✓ a render: true source never re-enters the shell-retry seam (rendered exactly once)");
}
